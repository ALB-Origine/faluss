...
final class Faluss_Link {
    ...
    public static function render_card( $attributes = array() ) {
        ...
        return 'immersive' === $attributes['presentation'] ? str_replace( 'faluss-link-card ', 'faluss-link-card faluss-link-card--presentation-immersive ', $markup ) : $markup;
    }
        ...
        <section class="faluss-link-studio">
            ...
            <form class="faluss-link-studio__form" method="post" enctype="multipart/form-data" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
                ...
                <div class="faluss-link-studio__workspace">
                    ...
                    <section id="faluss-studio-panel-style" role="tabpanel" aria-labelledby="faluss-studio-tab-style" data-fl-panel="style" hidden><?php self::preference_fields( $preferences, true ); ?></section>
    ...
    private static function save_preferences( $faluss_id, $post ) {
        ...
        $payload = array( 'networks' => self::socials( wp_unslash( $post['social_networks'] ?? $post['social_links'] ?? array() ) ), 'alignment' => $pick( wp_unslash( $post['alignment'] ?? '' ), array( 'left', 'center' ), 'left' ), 'hero_transition_color' => $color ? $color : '#FFFDF5', 'hero_transition_intensity' => min( 100, max( 0, (int) ( $post['hero_transition_intensity'] ?? 82 ) ) ), 'hero_transition_position' => min( 100, max( 35, (int) ( $post['hero_transition_position'] ?? 72 ) ) ) );
        ...
        $old = self::prefs( $faluss_id ); $now = gmdate( 'Y-m-d H:i:s' );
    ...
    private static function card_markup( $profile, $preferences, $alignment = 'left', $preview = false ) {
        ...
        $style = sprintf( '--fl-hero-transition-color:%s;--fl-hero-transition-intensity:%d%%;--fl-hero-transition-position:%d%%;', $preferences['hero_transition_color'], (int) $preferences['hero_transition_intensity'], (int) $preferences['hero_transition_position'] ); ob_start();
    ...
    private static function prefs( $faluss_id ) {
        global $wpdb; $defaults = array( 'faluss_id' => '', 'cover_attachment_id' => 0, 'avatar_visible' => 1, 'name_weight' => 'bold', 'name_treatment' => 'strong', 'available' => 0, 'bio_mode' => 'editorial', 'announcement' => '', 'announcement_variant' => 'accent', 'social_links' => '[]', 'social_layout' => 'bubbles', 'link_style' => 'solid', 'alignment' => 'left', 'hero_transition_color' => '#FFFDF5', 'hero_transition_intensity' => 82, 'hero_transition_position' => 72 );
        ...
        if ( is_array( $payload ) && isset( $payload['networks'] ) ) { $preferences['alignment'] = self::align( $payload['alignment'] ?? $preferences['alignment'] ); $preferences['hero_transition_color'] = sanitize_hex_color( $payload['hero_transition_color'] ?? '' ) ?: $preferences['hero_transition_color']; $preferences['hero_transition_intensity'] = min( 100, max( 0, (int) ( $payload['hero_transition_intensity'] ?? $preferences['hero_transition_intensity'] ) ) ); $preferences['hero_transition_position'] = min( 100, max( 35, (int) ( $payload['hero_transition_position'] ?? $preferences['hero_transition_position'] ) ) ); }
        return $preferences;
    }
