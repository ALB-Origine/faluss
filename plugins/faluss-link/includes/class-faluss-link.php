...
final class Faluss_Link {
    ...
    const SOCIAL_APPEARANCES = array( 'automatic' => 'Automatique', 'black' => 'Noir', 'white' => 'Blanc', 'name' => 'Couleur du nom', 'official' => 'Couleurs officielles', 'full' => 'Logos complets' );
    ...
    private static function save_preferences( $faluss_id, $post ) {
        ...
        $payload = array( 'networks' => self::socials( wp_unslash( $post['social_networks'] ?? $post['social_links'] ?? array() ) ), 'alignment' => $pick( wp_unslash( $post['alignment'] ?? '' ), array( 'left', 'center' ), 'left' ), 'page_background' => $page_background ? $page_background : '#FFFDF5', 'hero_transition_color' => $color ? $color : '#FFFDF5', 'hero_transition_intensity' => min( 100, max( 0, (int) ( $post['hero_transition_intensity'] ?? 82 ) ) ), 'hero_transition_position' => min( 100, max( 35, (int) ( $post['hero_transition_position'] ?? 72 ) ) ), 'name_color' => $name_color, 'social_appearance' => $pick( wp_unslash( $post['social_appearance'] ?? '' ), array_keys( self::SOCIAL_APPEARANCES ), $old['social_appearance'] ) );
    ...
    private static function prefs( $faluss_id ) {
        global $wpdb; $defaults = array( 'faluss_id' => '', 'cover_attachment_id' => 0, 'avatar_visible' => 1, 'name_weight' => 'bold', 'name_treatment' => 'strong', 'available' => 0, 'bio_mode' => 'editorial', 'announcement' => '', 'announcement_variant' => 'accent', 'social_links' => '[]', 'social_layout' => 'bubbles', 'link_style' => 'solid', 'alignment' => 'left', 'page_background' => '#FFFDF5', 'hero_transition_color' => '#FFFDF5', 'hero_transition_intensity' => 82, 'hero_transition_position' => 72, 'name_color' => '#000000', 'social_appearance' => 'automatic' );
    ...
    private static function social_appearance( $value ) { return is_string( $value ) && isset( self::SOCIAL_APPEARANCES[ $value ] ) ? $value : 'automatic'; }
    ...
    private static function network_catalog_for_client() { $catalog = self::network_catalog(); foreach ( $catalog as $network => $settings ) { $attachment_id = (int) ( $settings['icon'] ?? 0 ); $catalog[ $network ]['logo'] = $attachment_id && wp_attachment_is_image( $attachment_id ) ? wp_get_attachment_image_url( $attachment_id, 'thumbnail' ) : ''; } return $catalog; }
    ...
    private static function icon( $network, $full = false ) {
        $catalog = self::network_catalog(); $attachment_id = (int) ( $catalog[ $network ]['icon'] ?? 0 );
        if ( $full && $attachment_id && wp_attachment_is_image( $attachment_id ) ) { return wp_get_attachment_image( $attachment_id, 'thumbnail', false, array( 'alt' => '', 'class' => 'faluss-link-card__network-icon' ) ); }
