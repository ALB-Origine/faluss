...
final class Faluss_Link {
    ...
    private static function save_preferences( $faluss_id, $post ) {
        ...
        $social_appearance = self::social_appearance( wp_unslash( $post['social_appearance'] ?? $old['social_appearance'] ) );
        $payload = array( 'networks' => self::socials( wp_unslash( $post['social_networks'] ?? $post['social_links'] ?? array() ) ), 'alignment' => $pick( wp_unslash( $post['alignment'] ?? '' ), array( 'left', 'center' ), 'left' ), 'page_background' => $page_background ? $page_background : '#FFFDF5', 'hero_transition_color' => $color ? $color : '#FFFDF5', 'hero_transition_intensity' => min( 100, max( 0, (int) ( $post['hero_transition_intensity'] ?? 82 ) ) ), 'hero_transition_position' => min( 100, max( 35, (int) ( $post['hero_transition_position'] ?? 72 ) ) ), 'name_color' => $name_color, 'social_appearance' => $social_appearance );
