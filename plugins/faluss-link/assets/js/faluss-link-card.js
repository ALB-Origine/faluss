(function () {
    ...
    function refresh(card) {
        ...
        var surface = effectiveSurface(card);
        card.querySelectorAll('.faluss-link-card__name,.faluss-link-card__section-title,.faluss-link-card__media-teaser-copy h3').forEach(function (node) { readable(node, '--fl-title-color', surface); });
        card.querySelectorAll('.faluss-link-card__handle,.faluss-link-card__bio,.faluss-link-card__content-text,.faluss-link-card__media-teaser-copy p').forEach(function (node) { readable(node, '--fl-secondary-color', surface); });
        socialMode(card, surface);
    }
    ...
    window.FalussLinkCard = { refresh: refresh, initialize: initialize, contrastRatio: contrastRatio, bestContrastColor: bestContrastColor };
