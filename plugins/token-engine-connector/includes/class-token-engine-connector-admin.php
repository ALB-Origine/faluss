...
final class Token_Engine_Connector_Admin {
    ...
    public static function boot() {
        ...
        add_action( 'admin_post_token_engine_connector_test_daily_reward', array( __CLASS__, 'test_daily_reward' ) );
        add_action( 'admin_post_token_engine_connector_test_entitlements', array( __CLASS__, 'test_entitlements' ) );
    }
    ...
    /** Inspects daily-reward readiness without sending a subject or claiming a gain. */
    public static function test_daily_reward() {
        self::guard( 'token_engine_connector_test_daily_reward' );
        $result = Token_Engine_Connector_Service::daily_reward_diagnostic();
        set_transient( self::result_key( 'daily_reward' ), $result, MINUTE_IN_SECONDS );
        self::redirect( 'diagnostic', 'daily_reward_tested' );
    }

    /** Entitlement diagnosis reads no grant and never creates a local record. */
    public static function test_entitlements() {
        self::guard( 'token_engine_connector_test_entitlements' );
        $result = Token_Engine_Connector_Service::entitlements_diagnostic();
        set_transient( self::result_key( 'entitlements' ), $result, MINUTE_IN_SECONDS );
        self::redirect( 'diagnostic', 'entitlements_tested' );
    }
    ...
    private static function diagnostic_page() {
        ...
        $daily_reward = self::take_result( 'daily_reward' );
        $core_valid = (bool) get_transient( self::core_valid_key() );
        ?><h2>Diagnostic</h2><p>Les contrôles sont non mutatifs. Aucun secret, jeton, en-tête d’autorisation, Faluss ID ni réponse distante brute n’est affiché.</p><section class="token-engine-connector-admin__diagnostic-section" aria-labelledby="token-engine-connector-core-title"><h3 id="token-engine-connector-core-title">Connexion au Core</h3><p>Valide l’URL du site Core, détecte la forme REST, puis vérifie route, identité, protocole, client, secret, <code>wallet.read</code>, l’état de <code>reward.claim</code> et le jeton court. Ce contrôle ne dépend pas d’une session Faluss, d’un profil ou d’un solde.</p><?php self::core_result( $core ); ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="token_engine_connector_test_core"><?php wp_nonce_field( 'token_engine_connector_test_core', 'token_engine_connector_nonce' ); ?><?php submit_button( 'Tester la connexion au Core', 'secondary' ); ?></form></section><section class="token-engine-connector-admin__diagnostic-section" aria-labelledby="token-engine-connector-reward-title"><h3 id="token-engine-connector-reward-title">Diagnostic gain quotidien</h3><p>Vérifie la connexion Core, <code>reward.claim</code>, la règle <code>daily_reward</code>, l’acceptation de sa portée globale et, si un membre est connecté, la disponibilité de son sujet. Aucun gain n’est réclamé.</p><?php self::daily_reward_result( $daily_reward ); ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="token_engine_connector_test_daily_reward"><?php wp_nonce_field( 'token_engine_connector_test_daily_reward', 'token_engine_connector_nonce' ); ?><?php submit_button( 'Diagnostiquer le gain quotidien', 'secondary' ); ?></form></section><section class="token-engine-connector-admin__diagnostic-section" aria-labelledby="token-engine-connector-subject-title"><h3 id="token-engine-connector-subject-title">Diagnostic Sujet Faluss</h3><p>Disponible seulement après une connexion Core valide. Il vérifie l’adaptateur Identity, le profil actif de l’utilisateur WordPress courant et la disponibilité d’un sujet, sans afficher le Faluss ID.</p><?php self::subject_result( $subject ); ?><?php if ( $core_valid ) : ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="token_engine_connector_test_subject"><?php wp_nonce_field( 'token_engine_connector_test_subject', 'token_engine_connector_nonce' ); ?><?php submit_button( 'Diagnostiquer le sujet Faluss', 'secondary' ); ?></form><?php else : ?><p class="description">Validez d’abord la connexion au Core.</p><?php endif; ?></section><?php
    }

    /** Reads entitlement availability only; it never exposes a subject or mutates the Core. */
    private static function entitlements_page() {
        $result = self::take_result( 'entitlements' );
        ?><h2>Diagnostic des droits</h2><p>Ce contrôle vérifie la lecture centralisée des droits par le Core. Il ne crée ni droit local, ni attribution, ni écriture de ledger.</p><section class="token-engine-connector-admin__diagnostic-section" aria-labelledby="token-engine-connector-entitlements-title"><h3 id="token-engine-connector-entitlements-title">Droits de thème</h3><p>Vérifie la connexion au Core, la permission <code>entitlements.read</code>, la lecture des définitions du projet et, pour le membre connecté, la disponibilité du sujet sans afficher son identifiant.</p><?php self::entitlements_result( $result ); ?><form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>"><input type="hidden" name="action" value="token_engine_connector_test_entitlements"><?php wp_nonce_field( 'token_engine_connector_test_entitlements', 'token_engine_connector_nonce' ); ?><?php submit_button( 'Diagnostiquer les droits', 'secondary' ); ?></form></section><?php
    }

    private static function core_result( $result ) {
        if ( ! is_array( $result ) ) { return; }
        $success = ! empty( $result['connected'] );
        ?><div class="notice <?php echo $success ? 'notice-success' : 'notice-error'; ?>"><p><?php echo esc_html( self::core_message( $result['code'] ?? '' ) ); ?><?php if ( $success && ! empty( $result['project_key'] ) ) : ?> <?php echo esc_html( 'Projet : ' . $result['project_key'] . '.' ); ?><?php endif; ?></p><?php if ( $success ) : ?><ol class="token-engine-connector-admin__steps"><?php foreach ( self::step_labels() as $key => $label ) : ?><li><?php echo esc_html( $label . ' : ' . ( $result['steps'][ $key ] ?? 'non validée' ) ); ?></li><?php endforeach; ?></ol><p class="description">Récompense quotidienne (<code>reward.claim</code>) : <?php echo in_array( Token_Engine_Connector_Service::PERMISSION_REWARD_CLAIM, (array) ( $result['permissions'] ?? array() ), true ) ? 'accordée.' : 'absente : la réclamation publique reste indisponible jusqu’à son autorisation côté Core.'; ?></p><?php elseif ( ! empty( $result['stage'] ) ) : ?><p class="description">Étape bloquante : <?php echo esc_html( self::step_labels()[ $result['stage'] ] ?? $result['stage'] ); ?></p><?php endif; ?><?php if ( ! empty( $result['diagnostic_id'] ) ) : ?><p class="description">ID de diagnostic : <code><?php echo esc_html( $result['diagnostic_id'] ); ?></code></p><?php endif; ?></div><?php
    }
    ...
    private static function daily_reward_result( $result ) {
        if ( ! is_array( $result ) ) { return; }
        $state = in_array( $result['rule_state'] ?? '', array( 'ready', 'rule_unavailable', 'configuration_invalid', 'transient_error' ), true ) ? $result['rule_state'] : 'transient_error';
        $message = array( 'ready' => 'Le gain quotidien est prêt : la règle globale peut être utilisée par cette surface autorisée.', 'rule_unavailable' => 'La règle quotidienne est absente, inactive ou ne respecte pas la portée globale attendue.', 'configuration_invalid' => 'La configuration Core ou le projet client n’est pas prêt.', 'transient_error' => 'Le diagnostic quotidien ne peut pas joindre le Core pour le moment.' );
        ?><div class="notice <?php echo 'ready' === $state ? 'notice-success' : 'notice-error'; ?>"><p><?php echo esc_html( $message[ $state ] ); ?></p><dl class="token-engine-connector-admin__diagnostic"><dt>Connexion Core</dt><dd><?php echo ! empty( $result['core_connected'] ) ? 'Validée' : 'Non validée'; ?></dd><dt>Permission reward.claim</dt><dd><?php echo ! empty( $result['reward_claim_authorized'] ) ? 'Accordée' : 'Absente ou révoquée'; ?></dd><dt>Règle daily_reward</dt><dd><?php echo 'ready' === $state ? 'Active et compatible' : esc_html( $state ); ?></dd><dt>Portée globale</dt><dd><?php echo ! empty( $result['global_scope_accepted'] ) ? 'Acceptée : toutes les surfaces autorisées' : 'Non validée'; ?></dd><dt>Sujet Faluss courant</dt><dd><?php echo empty( $result['subject_checked'] ) ? 'Non vérifié : aucun membre connecté' : ( ! empty( $result['subject_available'] ) ? 'Disponible sans affichage de valeur' : 'Indisponible' ); ?></dd></dl></div><?php
    }

    private static function entitlements_result( $result ) {
        if ( ! is_array( $result ) ) { return; }
        $ready = ! empty( $result['core_connected'] ) && ! empty( $result['entitlements_read_authorized'] ) && ! empty( $result['definitions_readable'] );
        ?><div class="notice <?php echo $ready ? 'notice-success' : 'notice-error'; ?>"><p><?php echo $ready ? 'Lecture centralisée des droits prête.' : 'Lecture centralisée des droits indisponible.'; ?></p><dl class="token-engine-connector-admin__diagnostic"><dt>Connexion Core</dt><dd><?php echo ! empty( $result['core_connected'] ) ? 'Validée' : 'Non validée'; ?></dd><dt>Permission entitlements.read</dt><dd><?php echo ! empty( $result['entitlements_read_authorized'] ) ? 'Accordée' : 'Absente ou révoquée'; ?></dd><dt>Définitions de droit</dt><dd><?php echo ! empty( $result['definitions_readable'] ) ? 'Lisibles depuis le Core' : 'Indisponibles'; ?></dd><dt>Sujet Faluss courant</dt><dd><?php echo empty( $result['subject_checked'] ) ? 'Non vérifié : aucun membre connecté' : ( ! empty( $result['subject_available'] ) ? 'Disponible sans affichage de valeur' : 'Indisponible' ); ?></dd></dl></div><?php
    }
    ...
    private static function core_message( $code ) {
        $messages = array(
            ...
            'connector_permission_reward_claim_missing' => 'Permission reward.claim absente ou révoquée côté Core.',
            'connector_permission_entitlements_read_missing' => 'Permission entitlements.read absente ou révoquée côté Core.',
    ...
    private static function tabs() { return array( 'configuration' => 'Configuration', 'diagnostic' => 'Diagnostic', 'entitlements' => 'Droits' ); }
