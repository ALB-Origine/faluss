...
class Faluss_Production_Reset {
    ...
    private static function identity_attachments( $tables, $profiles, $candidate_ids ) {
        global $wpdb;
        $member_ids = array();
        foreach ( $profiles as $profile ) {
            $member_ids[ $profile['faluss_id'] ] = absint( $profile['wp_user_id'] );
        }
        $attachment_ids = self::author_attachments( $candidate_ids );
        if ( is_wp_error( $attachment_ids ) ) {
            return $attachment_ids;
        }
        $media_queries = array(
            array( 'table' => $tables['faluss_identity_public_profiles'], 'field' => 'avatar_attachment_id' ),
            array( 'table' => $tables['faluss_link_cards'], 'field' => 'cover_attachment_id' ),
        );
        foreach ( $media_queries as $query ) {
            $rows = $wpdb->get_results( "SELECT faluss_id, {$query['field']} AS attachment_id FROM {$query['table']} WHERE {$query['field']} IS NOT NULL", ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            if ( null === $rows ) {
                return new WP_Error( 'fpr_media_unreadable', 'Les références média ne sont pas vérifiables.' );
            }
            foreach ( $rows as $row ) {
                $user_id = $member_ids[ $row['faluss_id'] ] ?? 0;
                if ( ! $user_id ) {
                    return new WP_Error( 'fpr_media_owner_ambiguous', 'Une référence média sans membre Identity bloque le preflight.' );
                }
                if ( self::is_privileged_user( $user_id ) ) {
                    continue;
                }
                $attachment_ids[] = absint( $row['attachment_id'] );
            }
        }
        $blocks = $wpdb->get_results( "SELECT faluss_id, block_type, payload FROM {$tables['faluss_link_blocks']} WHERE block_type = 'media_teaser'", ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
        if ( null === $blocks ) {
            return new WP_Error( 'fpr_media_unreadable', 'Les teasers média ne sont pas vérifiables.' );
        }
        foreach ( $blocks as $block ) {
            $user_id = $member_ids[ $block['faluss_id'] ] ?? 0;
            if ( ! $user_id ) {
                return new WP_Error( 'fpr_media_owner_ambiguous', 'Un teaser média sans membre Identity bloque le preflight.' );
            }
            if ( self::is_privileged_user( $user_id ) ) {
                continue;
            }
            $payload = json_decode( $block['payload'], true );
            if ( ! is_array( $payload ) || empty( $payload['attachment_id'] ) || ! is_numeric( $payload['attachment_id'] ) ) {
                return new WP_Error( 'fpr_media_payload_ambiguous', 'Un teaser média membre est ambigu ; le preflight est bloqué.' );
            }
            $attachment_ids[] = absint( $payload['attachment_id'] );
        }
        return self::validate_attachments( array_unique( array_filter( $attachment_ids ) ), $candidate_ids );
    }
    ...
    private static function validate_attachments( $attachment_ids, $candidate_ids ) {
        ...
        foreach ( $attachment_ids as $attachment_id ) {
            ...
            if ( ! $attachment || 'attachment' !== $attachment->post_type || ! in_array( absint( $attachment->post_author ), $candidate_ids, true ) ) {
                return new WP_Error( 'fpr_media_ambiguous', 'Un média est ambigu ou partagé ; le preflight est bloqué.' );
            }
