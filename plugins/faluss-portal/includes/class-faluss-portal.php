...
final class Faluss_Portal {
    ...
    private static function render_hub_daily_action( $daily_reward ) {
        ...
        if ( 'claimable' === $status ) {
            ...
            echo '<button class="faluss-portal__app-open faluss-portal__app-open--reward" type="submit" data-faluss-portal-hub-daily-submit aria-label="Gain quotidien : 20 Points Faluss">' . self::hub_pf_badge() . '<span class="faluss-portal__hub-daily-amount" data-faluss-portal-hub-daily-amount>20</span></button>';
            ...
            echo '<span class="faluss-portal__app-open faluss-portal__app-open--reward" role="status" aria-label="Gain quotidien déjà reçu : 20 Points Faluss">' . self::hub_pf_badge() . '<span class="faluss-portal__hub-daily-amount" aria-hidden="true">20</span></span>';
        } else {
            echo '<span class="faluss-portal__app-open" aria-hidden="true">' . self::external_link_icon() . '</span>';
        }
        echo '</span>';
