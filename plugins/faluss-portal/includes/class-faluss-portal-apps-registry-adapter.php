...
final class Faluss_Portal_Apps_Registry_Adapter {
    ...
    private static function valid_daily_document( $daily, $status ) {
        ...
        if ( ! self::exact_keys( $daily['reward'], array( 'amount_pf', 'economic_class', 'label' ) ) || 20 !== $daily['reward']['amount_pf'] || 'earned' !== $daily['reward']['economic_class'] || '20 PF' !== $daily['reward']['label'] || ! self::exact_keys( $daily['period'], array( 'type', 'timezone', 'logical_date' ) ) || 'daily' !== $daily['period']['type'] || 'Europe/Paris' !== $daily['period']['timezone'] || ! is_string( $daily['period']['logical_date'] ) || 1 !== preg_match( '/^\d{4}-\d{2}-\d{2}$/D', $daily['period']['logical_date'] ) ) {
            return false;
        }
