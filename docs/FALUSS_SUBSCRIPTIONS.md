...
Checkout est hébergé par Stripe : abonnement, un Price, quantité 1, carte obligatoire, adresse de facturation obligatoire et sauvegardée automatiquement sur le Customer Stripe pour Stripe Tax, automatic_tax, essai 15 jours et annulation sans moyen de paiement valide. Le retour navigateur est uniquement un contrôleur no-store : il valide le state opaque, l’expiration et, en succès, la session Checkout locale, puis effectue un PRG vers `wp-admin/admin.php?page=faluss-subscriptions&tab=sandbox-test` avec une notice à usage unique. Un webhook signé peut avoir fait passer la session locale de `open` à `completed` avant ce retour : les deux états restent valides pour le même state et la même session, sans que le navigateur n’apporte de preuve. La version 0.2.7 corrige le faux négatif qui refusait à tort ce statut `completed` déjà synchronisé. Si la souscription locale liée est déjà résolue `trialing`, la notice indique seulement cet état déjà canonique; sinon elle reste neutre dans l’attente de Stripe. Ni ce retour, ni sa notice ne décident d’un droit; seul un rechargement serveur autoritaire de Stripe peut le faire. Tout retour invalide, expiré ou annulé suit le même PRG sûr sans donnée fournisseur dans l’URL finale.
...
États normalisés : trialing, active, canceling, past_due, suspended, expired. L’essai exige une carte réellement relue et une fenêtre strictement égale à 15 jours, puis est consommé atomiquement par Faluss ID et empreinte dérivée de carte. La preuve accepte le moyen de paiement par défaut de la souscription ou, si Stripe ne l’a pas encore projeté sur celle-ci, celui du Customer relu par API; le moyen de paiement doit appartenir à ce Customer et fournir une empreinte de carte valide. `trialing` ne devient effectif que si cette preuve et l’enregistrement d’essai vérifié correspondent à la même souscription Stripe. past_due garde Pro au plus sept jours depuis le **premier** échec observé, sans glissement par relivraison. unpaid, paused, incomplete, incomplete_expired et les fins de période sont Gratuit. Les remboursements et litiges sont réconciliés, sans révocation automatique arbitraire.

La résolution UTC et à lecture conserve l’ordre : révocation conformité, attribution administrative, souscription `trialing` avec essai vérifié correspondant, essai valide, abonnement actif/à résilier, grâce past_due, puis Gratuit. Une erreur Stripe n’efface ni n’invente un droit.

### Provisionnement d’essai, annulation et reprise sûre (0.2.6)

Une livraison Stripe signée relit toujours la souscription, le Price, le Customer et le moyen de paiement courants avant de décider. La décision d’essai est maintenant canonique pour le couple **Faluss ID + référence de souscription Stripe** : après une première preuve valide, les livraisons ultérieures de cette même souscription réemploient l’essai déjà vérifié. Elles ne le consomment pas une seconde fois, ne le refusent pas à cause d’une représentation ultérieure du moyen de paiement, n’annulent pas la souscription et ne réécrivent pas à l’identique la souscription `trialing` ni sa notification.

La relation validée entre `checkout_sessions.provider_subscription_reference` et la souscription Stripe est persistée dès qu’elle est connue, après relecture serveur du Customer et de la souscription, y compris lors de la réconciliation test. Le défaut observé après 0.2.5 venait de l’ordre de l’annulation : le marqueur local `trial_ineligible` était écrit **après** `DELETE /v1/subscriptions/{id}`. Deux livraisons signées concurrentes de la même souscription pouvaient donc toutes deux envoyer ce DELETE, puis écrire des refus et audits identiques.

Une annulation distante liée à l’éligibilité d’essai reste limitée au seul cas établi où une empreinte dérivée de moyen de paiement déjà vérifiée est utilisée par **une autre** identité Faluss **et** où la souscription est liée au Checkout local correspondant. Avant tout appel Stripe, la version 0.2.6 exécute une mise à jour conditionnelle atomique du Checkout `completed` vers `trial_ineligible_pending`. Cette revendication durable est fondée sur le Checkout déjà lié au couple Faluss ID + souscription Stripe : un seul worker la gagne; les autres reconnaissent la décision et n’appellent jamais Stripe, ni ne dupliquent les audits. Le worker gagnant réalise au plus un DELETE, finalise le marqueur en `trial_ineligible`, puis enregistre une seule fois `stripe_trial_refused` et `stripe_subscription_canceled_for_trial_ineligibility`, avec `reason` et `justification=payment_fingerprint_already_consumed`. Un 404 Stripe `resource_missing` est un résultat terminal `already_canceled` du seul worker revendiquant : il n’engendre ni second DELETE ni nouvel audit. Une preuve manquante, une empreinte indisponible, une fenêtre invalide, une relation locale absente ou transitoire, et une seconde livraison du même Checkout ne demandent jamais d’annulation distante. Les refus conservent uniquement des codes sûrs, notamment `trial_already_consumed`, `payment_proof_missing`, `payment_fingerprint_missing`, `payment_fingerprint_already_consumed`, `trial_window_invalid` ou `trial_activation_failed`.

Une souscription Stripe dont l’état actuel est `canceled` reste `expired` : elle ne peut pas réactiver son essai historique lors de la réconciliation et ne reçoit aucun entitlement artificiel. Les anciens audits volontairement minimisés ne suffisent pas à attribuer rétrospectivement un motif individuel sans enregistrer de donnée Stripe sensible; la correction s’applique aux nouveaux Checkouts et laisse une souscription déjà annulée à Gratuit.

Une livraison signée `customer.updated` ou `payment_method.*` reçue après la projection du moyen de paiement reprend une session Checkout locale correspondante, complète et relue auprès de Stripe; elle n’utilise jamais le snapshot de l’événement ni le retour navigateur. L’onglet **Sandbox test** propose également « Réconcilier le Checkout test » pour un `cs_…` déjà créé : il est test-only, exige une session locale ouverte ou déjà liée, le même Customer et une session Stripe complète, puis relit la souscription Stripe. Cette action n’est ni un grant administratif ni une confiance dans le navigateur; elle ne peut créer aucun droit si la souscription, le Price, l’essai ou la preuve de paiement est invalide.
