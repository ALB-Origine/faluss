...
### Provisionnement d’essai, annulation et reprise sûre (0.2.5)
...
La cause de régression identifiée dans le code était le seul appel interne à l’annulation Stripe, dans `Faluss_Subscriptions_Billing::refuse_trial()`. Il recevait auparavant des erreurs de fenêtre ou d’activation d’essai sans distinguer une seconde livraison du même abonnement d’une réutilisation démontrée de carte par une autre identité. La relation validée entre `checkout_sessions.provider_subscription_reference` et la souscription Stripe n’était en outre pas persistée. La version 0.2.5 lie cette référence après relecture serveur du Customer et de la souscription, y compris lors de la réconciliation test.

Une annulation distante liée à l’éligibilité d’essai est désormais limitée au seul cas établi où une empreinte dérivée de moyen de paiement déjà vérifiée est utilisée par **une autre** identité Faluss **et** où la souscription est liée au Checkout local correspondant. Cette demande est auditée une fois sous `stripe_subscription_canceled_for_trial_ineligibility`, avec uniquement `reason=payment_fingerprint_already_consumed` et un résultat sûr; le Checkout devient alors le marqueur local `trial_ineligible` qui déduplique toute relivraison suivante de cette souscription. Une preuve manquante, une empreinte indisponible, une fenêtre invalide, une relation locale absente ou transitoire, et une seconde livraison du même Checkout ne demandent jamais d’annulation distante. Les refus conservent uniquement des codes sûrs, notamment `trial_already_consumed`, `payment_proof_missing`, `payment_fingerprint_missing`, `payment_fingerprint_already_consumed`, `trial_window_invalid` ou `trial_activation_failed`.
