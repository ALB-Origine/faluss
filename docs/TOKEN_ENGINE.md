...
Les routes privées supplémentaires sont `POST /connector/reward/offer`, `POST /connector/reward/status` et `POST /connector/reward/claim`. Elles exigent HTTPS et `reward.claim`. L’offre ne retourne que le montant et le code d’unité de la règle configurée, sans sujet et sans écriture. Les deux autres ne retournent que l’état du sujet authentifié par le connecteur (`eligible`, `claimed` ou `unavailable`), le montant et le code d’unité autorisés, la prochaine disponibilité et la projection de solde du projet. Elles ne révèlent jamais d’information sur un autre sujet.
