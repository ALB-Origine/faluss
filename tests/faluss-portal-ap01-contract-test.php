...
foreach ( array(
    ...
    ap01_assert( 1 === preg_match( $mobile_rule, $css, $mobile_match ) && false !== strpos( $mobile_match[1], '--faluss-app-symbol-width: ' . $size[0] ) && false !== strpos( $mobile_match[1], '--faluss-app-symbol-height: ' . $size[1] ), 'AP-01E must preserve the exact mobile rendered-symbol dimensions for ' . $slug . '.' );
}
ap01_assert( 1 === preg_match( '/\.faluss-portal__app-card\s*\{[^}]*--faluss-app-logo-column-width:\s*22\.7px;/s', $css ) && 1 === preg_match( '/@media \(max-width:\s*720px\).*?\.faluss-portal__app-card\s*\{[^}]*--faluss-app-logo-column-width:\s*18px;/s', $css ), 'AP-01E must use the Hub geometry as the single shared logo column on desktop and mobile.' );
ap01_assert( 1 === preg_match( '/data-faluss-app="me"\]\s*\{[^}]*--faluss-app-symbol-width:\s*22\.7px;[^}]*--faluss-app-symbol-height:\s*53px;[^}]*--faluss-app-asset-width:\s*53px;[^}]*--faluss-app-asset-height:\s*53px;/s', $css ), 'AP-01E must keep the complete official Faluss Me asset separate from its desktop layout column.' );
ap01_assert( 1 === preg_match( '/@media \(max-width:\s*720px\).*?data-faluss-app="me"\]\s*\{[^}]*--faluss-app-symbol-width:\s*18px;[^}]*--faluss-app-symbol-height:\s*48px;[^}]*--faluss-app-asset-width:\s*48px;[^}]*--faluss-app-asset-height:\s*48px;/s', $css ), 'AP-01E must center the complete Faluss Me asset inside the 18px mobile Hub column without making its canvas a grid track.' );
...
ap01_assert( 1 === preg_match( '/data-faluss-app="me"\]\s+\.faluss-portal__app-logo img\s*\{[^}]*transform:\s*scale\(1\.5\);[^}]*transform-origin:\s*center\s+center;/s', $css ), 'AP-01F must increase only the Faluss Me painted symbol by 50 percent around its existing center.' );
...
foreach ( array( 'AP-01', 'AP-01E', 'Faluss Hub', 'Faluss Me', 'Bientôt disponible', 'Aucun appel', 'inter-domaine', 'preuve locale', '--faluss-app-logo-column-width', 'fenêtre de découpe' ) as $needle ) {
