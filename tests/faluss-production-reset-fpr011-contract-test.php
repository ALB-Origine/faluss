...
// A 0.1.0 armament is invalidated while loading 0.1.1; boot only registers handlers.
$fpr011_options = array(
    Faluss_Production_Reset::OPTION_ARMED => 1,
    Faluss_Production_Reset::OPTION_ARMED_VERSION => '0.1.0',
    Faluss_Production_Reset::OPTION_LOCKED => 0,
);
$fpr011_hooks = array();
Faluss_Production_Reset::boot();
fpr011_assert( 0 === $fpr011_options[ Faluss_Production_Reset::OPTION_ARMED ] && '0.1.1' === $fpr011_options[ Faluss_Production_Reset::OPTION_ARMED_VERSION ] && false === $armed->invoke( null ), 'An armament created under 0.1.0 must be invalidated and require explicit re-arm under 0.1.1.' );
...
fpr011_assert( false === strpos( $service, 'CREATE TABLE' ) && false === strpos( $service, 'dbDelta' ) && false === strpos( $service, 'migrate_' ), 'FPR-01.1 must add no table or migration.' );

echo 'FPR-01.1 PF preflight and armament invalidation contract: OK' . PHP_EOL;
