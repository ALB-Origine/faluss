...
foreach ( array( "'automatic' => 'Automatique'", "'black' => 'Noir'", "'white' => 'Blanc'", "'name' => 'Couleur du nom'", "'official' => 'Couleurs officielles'", "'full' => 'Logos complets'", 'social_appearance', 'network_catalog_for_client', 'faluss-link-card__social--appearance-' ) as $needle ) { fl10_assert( false !== strpos( $link . $editor . $css, $needle ), 'A required social appearance mode is missing: ' . $needle ); }
foreach ( array( 'relativeLuminance', 'contrastRatio', 'threshold = 4.5', 'bestContrastColor', 'effectiveSurface', '--fl-title-color-resolved', '--fl-secondary-color-resolved' ) as $needle ) { fl10_assert( false !== strpos( $card_script . $css, $needle ), 'The contrast resolver does not use the effective surface: ' . $needle ); }
...
fl10_assert( 'automatic' === $appearance->invoke( null, 'invalid' ) && 'official' === $appearance->invoke( null, 'official' ), 'Social appearance persistence is strictly allowlisted with an automatic default.' );
