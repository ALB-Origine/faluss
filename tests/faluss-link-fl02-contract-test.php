...
foreach(array('function icon','<svg aria-hidden="true"','aria-label="<?php echo esc_attr(ucfirst($s[', 'rel="noopener noreferrer nofollow"', 'faluss-link-card__links') as $needle)fl02_assert(false!==strpos($source,$needle),'Missing safe public social or link rendering: '.$needle);
foreach(array('svg(network)','aria-label', 'faluss-link-card--avatar-yes', 'faluss-link-card--avatar-no', 'function links', "rel:'noopener noreferrer nofollow'") as $needle)fl02_assert(false!==strpos($js,$needle),'Missing complete live-preview synchronization: '.$needle);
foreach(array('--faluss-action','--faluss-action-hover','--faluss-action-active','focus-visible','prefers-reduced-motion','mask-image','faluss-link-studio__tabs') as $needle)fl02_assert(false!==strpos($css,$needle),'Missing Faluss UI interaction token: '.$needle);
