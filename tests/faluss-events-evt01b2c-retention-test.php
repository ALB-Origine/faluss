...
/* Compare the five generated DDL statements against the mandatory base class. */
$base_source = shell_exec( 'git -C ' . escapeshellarg( $root ) . ' show 8c6518a503af3710a1fa513980581bd0ca3be412:plugins/faluss-events/includes/class-faluss-events-schema.php' );
evt01b2c_assert( is_string( $base_source ) && false !== strpos( $base_source, 'final class Faluss_Events_Schema' ), 'Mandatory base schema source must be available.' );
$base_source = preg_replace( '/^<\?php\s*/', '', $base_source, 1 );
$base_source = str_replace( 'Faluss_Events_Schema', 'EVT01B2C_Base_Schema', $base_source );
eval( $base_source );
$wpdb = $legacy;
foreach ( array( 'catalogs', 'events', 'outbox', 'inbox', 'consumer_deliveries' ) as $name ) {
    $table_method = 'consumer_deliveries' === $name ? 'consumer_deliveries_table' : $name . '_table';
    $table = call_user_func( array( 'Faluss_Events_Schema', $table_method ) );
    $base_ddl = evt01b2c_private( 'EVT01B2C_Base_Schema', 'create_query', array( $name, $table ) );
    $current_ddl = evt01b2c_private( 'Faluss_Events_Schema', 'create_query', array( $name, $table ) );
    evt01b2c_assert( $base_ddl === $current_ddl, 'Historical DDL must remain byte-for-byte identical: ' . $name );
}
...
$retention_source = file_get_contents( $root . '/plugins/faluss-events/includes/class-faluss-events-retention.php' );
...
$federation_status = 1; exec( 'git -C ' . escapeshellarg( $root ) . ' diff --quiet 8c6518a503af3710a1fa513980581bd0ca3be412 -- plugins/faluss-federation', $unused, $federation_status );
evt01b2c_assert( 0 === $federation_status, 'The complete Federation tree must remain byte-for-byte identical to the mandatory base.' );
$base_retention_status = 0; exec( 'git -C ' . escapeshellarg( $root ) . ' cat-file -e 8c6518a503af3710a1fa513980581bd0ca3be412:plugins/faluss-events/includes/class-faluss-events-retention.php 2>&1', $unused, $base_retention_status );
evt01b2c_assert( 0 !== $base_retention_status, 'Mandatory base must fail EVT-01B.2C because the production retention class is absent.' );
