...
$identity_session_source = file_get_contents( $root . '/plugins/faluss-identity/includes/class-faluss-identity-member-session.php' );
...
require_once $root . '/plugins/faluss-identity/includes/class-faluss-identity-member-session.php';
...
require_once $root . '/plugins/faluss-identity/includes/class-faluss-identity-authorization.php';
...
require_once $root . '/plugins/faluss-identity/includes/class-faluss-identity-sso-clients-admin.php';
...
$schema_source = file_get_contents( $root . '/plugins/faluss-identity/includes/class-faluss-identity-schema.php' );
...
require_once $root . '/plugins/faluss-identity-client/includes/class-faluss-identity-client.php';
fi06_sso_assert( 3600 === Faluss_Identity_Client::member_cookie_expiration( 172800, 10, false ), 'A linked Faluss.com member receives a one-hour local session.' );
fi06_sso_assert( 172800 === Faluss_Identity_Client::member_cookie_expiration( 172800, 11, false ), 'A privileged Faluss.com user keeps WordPress session semantics.' );
fi06_sso_assert( 172800 === Faluss_Identity_Client::member_cookie_expiration( 172800, 12, false ), 'A local role combination keeps WordPress session semantics.' );
foreach ( array( 'faluss_identity_client_continue', 'MEMBER_SESSION_TTL = 3600', 'auth_cookie_expiration', 'is_local_member_session', 'START TRANSACTION', 'FOR UPDATE', 'consumed_at IS NULL', "'S256'", 'wp_safe_redirect', "'httponly' => true", "'samesite' => 'Lax'", 'FALUSS_IDENTITY_CLIENT_SECRET' ) as $needle ) {
    fi06_sso_assert( false !== strpos( $client_source, $needle ), 'Client continuation or SSO invariant is missing: ' . $needle );
}
fi06_sso_assert( strpos( $client_source, 'if ( self::is_local_member_session() )' ) < strpos( $client_source, "'response_type' => 'code'" ), 'An active local member avoids an unnecessary SSO redirection.' );
fi06_sso_assert( strpos( $client_source, 'if ( null === $row || ! self::is_opaque( $code ) )') < strpos( $client_source, 'wp_set_auth_cookie' ), 'Invalid, altered, expired or replayed callback state cannot open a local session.' );
...
fi06_sso_assert( false !== strpos( $protocol, 'Cause constatée et durée' ) && false !== strpos( $protocol, 'first_party' ) && false !== strpos( $client_docs, 'faluss_identity_client_continue' ), 'FI-06 session and first-party SSO decisions are documented.' );
