...
function an01b3_git_status( $root, $arguments ) {
    $output = array(); $status = 1;
    exec( 'git -C ' . escapeshellarg( $root ) . ' ' . $arguments . ' 2>&1', $output, $status );
    return array( $status, implode( "\n", $output ) );
}

$root = dirname( __DIR__ );
$base = '6bb57aa1940a7217fc0194be34b0631d495236eb';
$paths = array(
    $root . '/plugins/faluss-apps-registry/includes/class-faluss-apps-registry-manifest-validator.php',
    $root . '/plugins/faluss-events/includes/class-faluss-events-catalog-validator.php',
    $root . '/plugins/faluss-events/includes/class-faluss-events.php',
    $root . '/plugins/faluss-events/includes/class-faluss-events-engine.php',
    $root . '/plugins/faluss-portal/includes/class-faluss-portal-manifest.php',
    $root . '/plugins/faluss-portal/includes/class-faluss-portal-events-catalog.php',
    $root . '/plugins/faluss-link/includes/class-faluss-link-manifest.php',
    $root . '/plugins/faluss-link/includes/class-faluss-link-events-catalog.php',
    $root . '/plugins/faluss-analytics/includes/class-faluss-analytics-event-validator.php',
    $root . '/plugins/faluss-analytics/includes/class-faluss-analytics-consumer.php',
    $root . '/plugins/faluss-analytics/includes/class-faluss-analytics.php',
    $root . '/plugins/faluss-portal/includes/class-faluss-portal-events-runtime.php',
    $root . '/plugins/faluss-link/includes/class-faluss-link-events-runtime.php',
);
...
an01b3_assert( false !== strpos( $portal_bootstrap, 'Version: 0.1.24' ) && false !== strpos( $portal_bootstrap, "FALUSS_PORTAL_VERSION', '0.1.24'" ), 'Portal version must be exactly 0.1.24.' );
an01b3_assert( false !== strpos( $link_bootstrap, 'Version: 0.3.21' ) && false !== strpos( $link_bootstrap, "FALUSS_LINK_VERSION','0.3.21'" ), 'Link version must be exactly 0.3.21.' );
an01b3_assert( false !== strpos( $portal_bootstrap, 'Faluss_Portal_Events_Runtime::boot()' ) && false !== strpos( $link_bootstrap, 'Faluss_Link_Events_Runtime::boot()' ), 'Both owner runtimes must boot from their existing plugin bootstrap.' );

foreach ( array(
    'plugins/faluss-portal/includes/class-faluss-portal-manifest.php',
    'plugins/faluss-portal/includes/class-faluss-portal-events-catalog.php',
    'plugins/faluss-link/includes/class-faluss-link-manifest.php',
    'plugins/faluss-link/includes/class-faluss-link-events-catalog.php',
) as $unchanged ) {
    list( $status, $base_bytes ) = an01b3_git_status( $root, 'show ' . escapeshellarg( $base . ':' . $unchanged ) );
    an01b3_assert( 0 === $status && $base_bytes === rtrim( file_get_contents( $root . '/' . $unchanged ), "\r\n" ), 'AN-01B.2 manifest/catalog must remain byte-for-byte unchanged: ' . $unchanged );
}
list( $status ) = an01b3_git_status( $root, 'cat-file -e ' . escapeshellarg( $base . ':plugins/faluss-portal/includes/class-faluss-portal-events-runtime.php' ) );
an01b3_assert( 0 !== $status, 'The required Hub route runtime must not already exist at the base commit.' );
list( $status ) = an01b3_git_status( $root, 'cat-file -e ' . escapeshellarg( $base . ':plugins/faluss-link/includes/class-faluss-link-events-runtime.php' ) );
an01b3_assert( 0 !== $status, 'The required Me route and producer validator runtime must not already exist at the base commit.' );
...
$runtime_files = array(
    $root . '/plugins/faluss-portal/includes/class-faluss-portal-events-runtime.php',
    $root . '/plugins/faluss-link/includes/class-faluss-link-events-runtime.php',
    $root . '/plugins/faluss-portal/faluss-portal.php',
    $root . '/plugins/faluss-link/faluss-link.php',
);
$runtime_source = '';
...
foreach ( array(
    'plugins/faluss-events', 'plugins/faluss-federation', 'plugins/faluss-analytics', 'plugins/faluss-apps-registry',
    'plugins/faluss-identity', 'plugins/faluss-identity-client', 'plugins/faluss-master-profile',
    'plugins/token-engine', 'plugins/token-engine-connector', 'contracts',
) as $protected ) {
    list( $status ) = an01b3_git_status( $root, 'diff --quiet ' . $base . ' -- ' . escapeshellarg( $protected ) );
    an01b3_assert( 0 === $status, 'Protected tree must remain byte-for-byte identical to the base: ' . $protected );
}

echo 'AN-01B.3 closed producer/runtime contracts: ' . $an01b3_assertions . '/' . $an01b3_assertions . " assertions passed.\n";
