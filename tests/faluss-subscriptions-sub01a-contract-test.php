...
sub01a_assert( false !== strpos( $bootstrap, 'Plugin Name: Faluss Subscriptions' ) && false !== strpos( $bootstrap, "FALUSS_SUBSCRIPTIONS_VERSION', '0.1.1'" ), 'SUB-01A repair requires a standalone Faluss Subscriptions plugin at 0.1.1.' );
...
sub01a_assert( false !== strpos( $admin, 'Faluss Pro a été attribué jusqu’au %s.' ) && false !== strpos( $admin, "array( 'success', 'warning', 'error' )" ) && false !== strpos( $admin, "notice-' . esc_attr( \$type )" ), 'Grant, warning and failure notices must be explicit and readable.' );
sub01a_assert( false !== strpos( $entitlements, 'START TRANSACTION' ) && false !== strpos( $entitlements, 'ROLLBACK' ) && false !== strpos( $entitlements, 'grant_by_id' ) && false !== strpos( $entitlements, 'saved_grant_matches' ), 'Administrative grants must be transactional, audited and reread before success.' );
