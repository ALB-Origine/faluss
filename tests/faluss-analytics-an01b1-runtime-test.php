...
foreach ( array( 'schema', 'event-validator', 'consumer', 'read-model', 'retention', '' ) as $part ) {
    $path = '' === $part ? $root . '/plugins/faluss-analytics/includes/class-faluss-analytics.php' : $root . '/plugins/faluss-analytics/includes/class-faluss-analytics-' . $part . '.php';
    require_once $path;
}
...
/* Static scope and base proof: no parallel runtime, producer, transport, UI or forbidden tree change. */
$plugin_source = '';
...
$historical_head = 'a20f831c97e8e153658dabcbc31a2934fe3ad46d';
...
foreach ( array( 'plugins/faluss-events', 'plugins/faluss-federation', 'plugins/faluss-portal', 'plugins/faluss-link', 'plugins/faluss-apps-registry', 'plugins/faluss-identity', 'plugins/faluss-identity-client', 'plugins/token-engine', 'plugins/token-engine-connector', 'contracts' ) as $protected ) { $status = 1; exec( 'git -C ' . escapeshellarg( $root ) . ' diff --quiet ' . escapeshellarg( $base ) . ' ' . escapeshellarg( $historical_head ) . ' -- ' . escapeshellarg( $protected ), $unused, $status ); an01b1_assert( 0 === $status, 'AN-01B.1.1 protected tree must have stayed byte-for-byte identical in its historical commit: ' . $protected ); }
