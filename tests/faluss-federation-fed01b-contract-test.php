...
fed01b_contains_all( $bootstrap, array( 'Version: 0.2.0', "FALUSS_FEDERATION_VERSION', '0.2.0'", 'Requires at least: 6.4', 'Requires PHP: 7.4', "FALUSS_FEDERATION_SCHEMA_VERSION', '1'" ), 'bootstrap' );
...
fed01b_contains_all( $providers, array( 'diagnostic.read', 'manifest.read', 'read_model.read', 'event_catalog.read', "failure( 'not_available' )", 'register_manifest_provider', 'register_read_model_provider', 'register_event_catalog_provider' ), 'providers' );
...
fed01b_contains_all( $client, array( 'diagnostic_read', 'manifest_read', 'read_model_read', 'event_catalog_read', 'wp_remote_post', "'sslverify' => true", "'redirection' => 0", "'timeout' => 3", "'limit_response_size' => self::MAX_RESPONSE", 'FALUSS_FEDERATION_DIAGNOSTIC_TRACE', '[Faluss Federation trace] side=client stage=', 'validate_response' ), 'client' );
