...
foreach ( array( "'core_site_url'", 'normalise_site_url', "'https'", 'isset( $parts[\'query\'] )', 'isset( $parts[\'fragment\'] )', 'isset( $parts[\'user\'] )', 'wp-json|index', "'token' => 'connector/token'", "'diagnostic' => 'connector/diagnostic'", "'balance' => 'connector/balance'" ) as $needle ) {
    tec013_assert( false !== strpos( $service, $needle ), 'TEC-01.3 must accept only a canonical HTTPS site URL and derive complete Core routes: ' . $needle );
}
tec013_assert( false === strpos( $service, 'REST_SUFFIX' ) && false !== strpos( $service, 'migrate_legacy_site_url' ) && false !== strpos( $service, "add_query_arg( 'rest_route'" ), 'TEC-01.3 must not store a REST root or append onto a query string.' );
...
foreach ( array( 'URL du site Core', 'Mécanisme REST détecté', 'Route Core', 'Core Token Engine', 'Protocole', 'Credentials', 'Permission wallet.read', 'Jeton court', 'core_valid_key', 'subject_core_required' ) as $needle ) {
...
foreach ( array( 'connector/token', 'version de protocole', 'rest_route', 'URL HTTPS du site Core' ) as $needle ) {
