...
sub01b_assert( false !== strpos( $billing, "'payment_fingerprint_already_consumed'" ) && false !== strpos( $billing, 'stripe_subscription_canceled_for_trial_ineligibility' ) && false !== strpos( $billing, "if ( \$cancel && 'payment_fingerprint_already_consumed' === \$safe_reason && is_array( \$checkout ) )" ) && false !== strpos( $billing, "'trial_ineligible' === ( \$linked['session_status'] ?? '' )" ) && false !== strpos( $billing, 'link_checkout_from_subscription' ) && false !== strpos( $repository, 'link_checkout_subscription' ) && false !== strpos( $repository, 'mark_checkout_trial_ineligible' ), 'Only a demonstrated cross-identity payment-fingerprint collision with its local Checkout may cancel a Stripe trial, and that safe decision is idempotent.' );
