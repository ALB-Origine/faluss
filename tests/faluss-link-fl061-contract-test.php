...
foreach ( array( 'wp_register_script( self::IMMERSIVE_SCRIPT', "wp_enqueue_script( self::IMMERSIVE_SCRIPT )", 'get_script_depends() { return array( \'faluss-link-immersive\' ); }' ) as $needle ) { fl061_assert( false !== strpos( $link . $widgets, $needle ), 'The immersive script dependency is not declared for the card widget: ' . $needle ); }
